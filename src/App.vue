...
const initializeFromUrl = async () => {
  ...
  if (sessionId && sessionId !== 'chat') {
    ...
    try {
      const response = await fetch(`https://staging.services.leadconnectorhq.com/copilot/sessions?companyId=5DP4iH6HLkQsiKESj6rh&sessionId=${sessionId}&userId=rTTL2p40dZSG1EMs2TFQ`, {
        headers: {
...
const sendMessage = async () => {
  ...
  try {
    ...
    const response = await fetch('https://staging.services.leadconnectorhq.com/copilot/input?companyId=5DP4iH6HLkQsiKESj6rh&userId=rTTL2p40dZSG1EMs2TFQ', {
      method: 'POST',
      headers: {
